...
bool checkPrototypeSeed(std::list<std::string>& messages){
	std::vector<std::string> data = generateRandomStringFile(5000, 10, test_file);
...
bool checkBatchConsistency(std::list<std::string>& messages) {
	static const size_t sims = std::thread::hardware_concurrency() * 26 * 2;
	size_t msgs = messages.size();
	std::vector<std::string> data = generateRandomStringFile(1000, 7, test_file);
	T s;
	int seed = std::random_device()();
	...
	std::thread t(std::move(task), std::ref(s), test_file, sims/2, seed);
	while (f.wait_for(std::chrono::seconds(0)) == std::future_status::timeout) {
		srand(std::random_device()());
	}
	t.join();
	s.batch(test_file, sims/2, seed);

	auto results = s.getResults();
	if (results.size() < sims) {
...
size_t doSimulatorTests(std::ostream& out) {
	...
	out << "\n\033[0;1;4mCompetition class\033[0m: \n\n";
	out << "\t" << (allProjectCityMethods<Competition_class> ? "\033[32mAll methods present " : "\033[31mSome methods are missing. Make sure ALL prototype methods (including string constructor) are implemented for competition class") << std::endl;
	out << "\t" << (std::is_same<Prototype, Competition_class>::value ? "\033[33mCompetition class is still defined as Prototype. Change to Competition once implemented." : "") << std::endl;
	out << "\t" << (std::is_base_of<Prototype, Competition_class>::value ? "" : "\033[31mCompetition class should inherit from Prototype") << std::endl;
	return score;
}